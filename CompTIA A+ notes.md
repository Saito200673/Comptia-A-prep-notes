...
Step 2: Establish the probable cause
...
1. Contains moving parts

2. Slower compared to ssd
But there isn’t much data degradation over time

3. Used to store big chunks of data
  
4. The smallest unit is sectors

2].SSD(solid state drives)

1. Faster than hdd
   
2. Doesn’t contain moving parts
  
3. Data degrades much faster than hdd

4. Used to store os for faster booting times
  
5. The smallest unit is blocks
...
If there’s an white piece of plastic in the USB A it’s ver 1.1
...
There also PCIE (E-express ) for faster speeds
...
3.HDMI-high definition multimedia interface has two versions hdmi and mini hdmi

4.DISPLAY PORT(dedicated connector for display) same as hdmi has both display and mini display port
